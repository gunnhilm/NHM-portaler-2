...
function parseLoanData(data) {
  try {
    ...
    console.log('parseLoanData: ' + error);
      return false
  }
}
...
function getCorrectInfo (data) {
  try {
    ...
    return lenderInfo
      ...
      console.log('getCorrectInfo: ' + error);
      return false
  }
}
...
async function  requestLoan (formData, fileData) {
  try {
    const passedValidation = validateInput(formData)
    ...
    const requestEmail = await email(parsedData, invoiceFileName, shippingDocumentationFileName, data.loaneeInfo) //.catch(console.error);
    // email recite to customer
    const reciteEmail = await emailReceipt(parsedData, data.loaneeInfo, lenderInfo.senderEmail) //.catch(console.error);
 
    if(reciteEmail && requestEmail && parsedData && shippingDocumentationFileName && invoiceFileName && itemData && lenderInfo && data && passedValidation ) {
      console.log('detta blir sant');
      return true
    } else {
      console.log('dette blir feil');
      return false
    }
  } catch (error) {
    console.log('requestLoan: ' + error);
    return false
  }
}
